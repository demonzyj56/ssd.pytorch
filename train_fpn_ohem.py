...
from layers.modules.multibox_ohem_loss import MultiBoxOHEMLoss
...
parser.add_argument('--num_samples', default=128, type=int, help='Number of samples to back propagate per image')
parser.add_argument('--lamb', default=5, type=float, help='Multiplier for ohem classification loss')
...
criterion = MultiBoxOHEMLoss(num_classes, args.num_samples, args.jaccard_threshold, cfg['variance'])
...
def train():
    ...
    for iteration in range(args.start_iter, max_iter):
        ...
        loss = loss_l + args.lamb*loss_c
        loss.backward()
        optimizer.step()
        t1 = time.time()
        if iteration % 10 == 0:
            print('Timer: %.4f sec' % ((time.time()-tic)/10))
            print('iter ' + repr(iteration) + ' || loc loss: %.4f, conf loss: %.4f ||' % (loss_l.data[0], args.lamb*loss_c.data[0]), end=' ')
            tic = time.time()
    ...
    final_model = args.save_folder + 'voc_ssd_fpn_' + args.version + '.pth'
    print('Saving final model to {}'.format(final_model))
    torch.save(ssd_net.state_dict(), final_model)
