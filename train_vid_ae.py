...
import numpy as np
import time
from data.scripts.create_train_lists import random_train_file
...
parser.add_argument('--epochs', default=20, type=int, help='Number of epochs for each video sequence')
...
parser.add_argument('--lamb', default=0.01, type=float, help='Regularity weight for discriminative reconstruction loss')
...
ae_net = SSDReconstruction(ssd_net)
net = ae_net
...
def train():
    net.train()
    dataset = VIDDetection(train_sets, 'data/', VIDroot, transform=BaseTransform(ssd_dim, means))
    ...
    for e in range(args.epochs):
        ...
        for images, targets in data_loader:
            ...
            if iteration % 1 == 0:
                print('Iter: {:d}, loss: {:.4f}, net: {:.4f}s, loss: {:.4f}s, backward: {:.4f}s'.format(
                    iteration, sum([l.data[0] for l in losses]), t_net, t_loss, t_backward
                ))
    ...
    #         np.save('features_{}_{}'.format(train_sets[0], i), f)
