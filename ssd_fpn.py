...
class SSDFPN(nn.Module):
    ...
    def forward(self, x):
        ...
        for (x, n) in zip(sources, self.cfg):
            loc.append(self.loc(x)[:, :n*4, :, :].permute(
                0, 2, 3, 1).contiguous())
            conf.append(self.conf(x)[:, :n*self.num_classes, :, :].permute(
                0, 2, 3, 1).contiguous())
...
def build_ssd_fpn(phase, size=300, num_classes=21):
    ...
    fpn, loc, conf = multibox_head_shared(vgg_layers, extra_layers,
