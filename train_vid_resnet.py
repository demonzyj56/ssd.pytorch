...
from resnet.network import build_ssd
import numpy as np
import time
...
parser.add_argument('--size', default=320, type=int, help='use 300 or 512 as base size')
parser.add_argument('--version', default='resnet320', help='conv11_2(v2) or pool6(v1) as last layer')
...
parser.add_argument('--crop_factor', default=None, type=str, help='Crop factor for augmentation')
parser.add_argument('--voc_root', default=VOCroot, help='Location of VOC root directory')
parser.add_argument('--freeze_batchnorm', default=True, type=str2bool, help='Whether to freeze batchnorm layers during training')
...
ssd_net = build_ssd('train', ssd_dim, num_classes, 'resnet34',
...
def train():
    ...
    for iteration in range(args.start_iter, max_iter):
        ...
        if iteration % 10000 == 0:
            ...
            torch.save(ssd_net.state_dict(), 'weights/ssd{}_vid_resnet_{}.pth'.format(
                args.size, iteration
            ))
    final_snapshot = args.save_folder + 'vid_ssd_resnet_' + args.version + '.pth'
    print('Saving {}'.format(final_snapshot))
    #  torch.save(ssd_net.state_dict(), args.save_folder + 'vid_ssd_resnet_' + args.version + '.pth')
    torch.save(ssd_net.state_dict(), final_snapshot)
