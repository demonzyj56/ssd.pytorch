...
import numpy as np
import time
from data.scripts.create_train_lists import train_list_by_volume
...
parser.add_argument('--jaccard_threshold', default=0.7, type=float, help='Min Jaccard index for matching')
parser.add_argument('--batch_size', default=32, type=int, help='Batch size for training')
parser.add_argument('--resume', default=None, type=str, help='Resume from checkpoint')
parser.add_argument('--num_workers', default=4, type=int, help='Number of workers used in dataloading')
parser.add_argument('--iterations', default=120000, type=int, help='Number of training iterations')
parser.add_argument('--epochs', default=30, type=int, help='Number of epochs for each video sequence')
...
parser.add_argument('--volume', default='train/ILSVRC2015_VID_train_0000/ILSVRC2015_train_00043000', help='Volume to train on')
# parser.add_argument('--volume', default='train/ILSVRC2015_VID_train_0002/ILSVRC2015_train_00453002', help='Volume to train on')
...
train_sets = [train_list_by_volume(args.volume)]
ssd_dim = args.size
...
# optimizer = optim.SGD(ae_net.ae.parameters(), lr=args.lr, weight_decay=args.weight_decay, momentum=args.momentum)
...
def train():
    ...
    for e in range(args.epochs):
        ...
        for images, targets in data_loader:
            ...
            if args.snapshot > 0 and iteration % args.snapshot == 0:
                filename = 'weights/ssd{}_vid_ae_{}_{}.pth'.format(args.size, train_sets[0], iteration)
    ...
    if args.snapshot > 0:
        final_name = os.path.join(args.save_folder, 'vid_ssd_ae_{}_{}.pth'.format(train_sets[0], args.version))
...
def score():
    ...
    for i, (images, targets) in enumerate(dataset):
        ...
        all_losses[key] = loss
        print('{:d}/{:d} ({:s}), loss: {:.1f}, time: {:.3f}s'.format(
    ...
    save_txt = 'data/cache/results/reconstruction_losses_{}.txt'.format(train_sets[0])
    print('Saving losses results to {}'.format(save_txt))
    with open(save_txt, 'wt') as f:
        for k, v in sorted(all_losses.items(), key=lambda x: x[0]):
            f.write('{:s} {:.1f}\n'.format(k, v))
...
if __name__ == '__main__':
    ...
    saved_text = 'data/cache/results/reconstruction_loss_{}_sorted.txt'.format(train_sets[0])
