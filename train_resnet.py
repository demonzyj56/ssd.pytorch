...
parser.add_argument('--iterations', default=200000, type=int, help='Number of training iterations')
...
stepvalues = (80000, 140000, 180000, 200000)
#  stepvalues = (-1, )
gamma = args.gamma
...
if args.resume:
    ...
    # network has been initialized internally
    pass
...
def train():
    ...
    for iteration in range(args.start_iter-args.warm_up, max_iter):
        ...
        if iteration % 10 == 0:
            ...
            #        ' || Loc loss: %.4f, conf loss: %.4f ||' % (loss_l.data[0], loss_c.data[0]), end=' ')
            tic = time.time()
        if iteration % 10000 == 0:
            print('Saving state, iter:', iteration)
            torch.save(ssd_net.state_dict(), 'weights/ssd{}_resnet_0712_{}.pth'.format(
                args.size, iteration
            ))
    #  torch.save(ssd_net.state_dict(), args.save_folder + '' + args.version + '.pth')
    final_snapshot = args.save_folder + 'voc_ssd_resnet_' + args.version + '.pth'
