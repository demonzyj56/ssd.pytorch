...
class SSDContext(nn.Module):
    ...
    def __init__(self, phase, base, extras, head, num_classes, sz=300):
        super(SSDContext, self).__init__()
        self.phase = phase
    ...
    def forward(self, x):
        ...
        context_values = []
        for x, c in zip(sources, self.context):
            context_values.append(c(x))
        batch_size = sources[0].size(0)
        sources_with_context = []
        for s, cv in zip(sources, context_values):
            front = s[:batch_size//2] + cv[batch_size//2:]
            back = s[batch_size//2:] + cv[:batch_size//2]
            sources_with_context.append(
                F.relu(torch.cat((front, back), dim=0), inplace=True)
            )

        # apply multibox head to source layers
        for (s, x, l, c) in zip(sources, sources_with_context, self.loc, self.conf):
    ...
    def decision_buffered(self, sources, buffers=None):
        ...
        output = self.detect(
            locations.view(locations.size(0), -1, 4),  # loc preds
...
def build_ssd_context(phase, size=300, num_classes=21):
    assert phase in ('train', 'test')

    return SSDContext(phase, *multibox(vgg(base[str(size)], 3),
